<template>
  <div>
    ...
    <ImgBanner imgSrc="https://source.unsplash.com/5brvJbR1Pn8/1600x900">
      <div style="line-height:1.2em;font-size:4vw;text-align:center" slot="text" id="porth">Pictures</div>
    </ImgBanner>

    <v-flex xs12 text-xs-center round my-5>
      <v-btn v-on:click="showPortWrite" class="mx-2" fab dark color="cyan" id="write">
  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">Upload a picture</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
               ...
               <v-col cols="12">
                <v-text-field v-model="img" label="Image" required></v-text-field>
              </v-col>
              <v-col cols="12">
    ...
    <v-container>
      ...
      <v-layout>
        <v-flex xs12>
          <PortfolioList :limits="4" :load-more="true">
          </PortfolioList>
        </v-flex>
      </v-layout>
    </v-container>
...
export default {
  ...
  methods: {
    ...
    savePort() {
      ...
      var body = this.body;
      var img = this.img;
      FirebaseService.postPortfolio(title, body, img)
      this.dialog = false
    }
  }
}
