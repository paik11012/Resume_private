  ...
  <div class="team back" @click="reset">
    <div v-for="i in team.length" :key="i">
      <transition name="teams" >
        <img :src="imgsrc[i-1]" :class="team[i-1]" class="circle" v-if="sel == team[i-1] | all" @click="select(team[i-1])" style="z-index:20;">
        <div v-else :class="team[i-1]+'on'" class="circle mess" @click="select(team[i-1])"> {{ message[sel][i-1] }} </div>
